---
name: amp-execution
description: How a worker agent reads its ticket, does the work, posts progress notes, and completes or blocks a task
---
...
```
amp_get_task_kb(task_id=YOUR_ID)         # KB entries in this task's context chain
amp_search_kb(query="...", project_id=X) # search by relevant terms
```
...
```
amp_block_task(task_id=YOUR_ID, reason="""
Cannot proceed: [exact reason]
What I tried: [attempts]
What is needed: [specific requirement to unblock]
""")
```

Stop immediately. Do not guess or work around silently.
