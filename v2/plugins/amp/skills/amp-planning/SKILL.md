...
## How to review
1. Load the code-reviewer skill: the **code-reviewer** skill
...
covers — because Claude Code generates that list live from whatever is actually configured under
`${CLAUDE_PLUGIN_ROOT}/agents/*.md`. Read that list each time you plan. It is always accurate to what's
...
the **amp-agent-builder** skill to mint a new specialist — don't force-fit an existing one.
...
File conflict check: found 2 conflicts, both serialized —
  ...
  - #22 and #31 both touch .claude/skills/amp-kb/SKILL.md → added #31 depends on #22
```
