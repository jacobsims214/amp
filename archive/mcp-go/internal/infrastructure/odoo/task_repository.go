	...
	result, err := r.client.ExecuteKw(ctx, "amp.task", "read",
		...
		map[string]interface{}{
			"fields": []string{
				...
				"project_id", "epic_id", "story_id", "state",
				"dag_level", "dag_critical_path", "priority", "planned_hours",
				"agent_id", "agent_session", "dispatch_time",
...
func (r *taskRepository) Dispatch(ctx context.Context, taskID int, agentID string) error {
	...
	if err != nil {
		// Odoo raises ValueError with blocking task details — surface as-is so the
		// manager knows exactly which task IDs to complete first.
		return err
	}
	return nil
}
...
func (r *taskRepository) ListByStory(ctx context.Context, storyID int) ([]models.Task, error) {
	result, err := r.client.ExecuteKw(ctx, "amp.task", "search_read",
		...
		map[string]interface{}{
			"fields": []string{
				"name", "state", "dag_level", "dag_critical_path",
				"agent_id", "priority",
			},
		})
	if err != nil {
...
func (r *taskRepository) Delete(ctx context.Context, taskID int) error {
	_, err := r.client.Execute(ctx, "amp.task", "unlink", []int{taskID})
	if err != nil {
...
func (r *taskRepository) DeleteByProject(ctx context.Context, projectID int) (int, error) {
	// Find all task IDs in the project
	result, err := r.client.ExecuteKw(ctx, "amp.task", "search",
		[]interface{}{[][]interface{}{{"project_id", "=", projectID}}},
		map[string]interface{}{})
	if err != nil {
		return 0, fmt.Errorf("failed to search tasks: %w", err)
	}
	...
	_, err = r.client.Execute(ctx, "amp.task", "unlink", intIDs)
	if err != nil {
