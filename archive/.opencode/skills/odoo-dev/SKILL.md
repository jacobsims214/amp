...
## Keeping Go MCP in sync with Odoo model changes

When you remove a field from an Odoo model, you must also remove it from the Go MCP layer:

1. `mcp-go/internal/domain/models/*.go` — remove the struct field
2. `mcp-go/internal/infrastructure/odoo/*_repository.go` — remove from the `fields` list in `ExecuteKw` calls AND from the `mapTo*` function
3. Rebuild: `docker compose up -d --build`

Failing to do this causes `ValueError: Invalid field 'field_name' on 'model.name'` on any read/create that touches that model.

Check for drift with:
```bash
grep -n "actual_hours\|<removed_field>" \
  mcp-go/internal/domain/models/*.go \
  mcp-go/internal/infrastructure/odoo/*.go
```
