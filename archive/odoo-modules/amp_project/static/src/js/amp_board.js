...
class AmpBoard extends Component {
    ...
    _subscribeBus(projectId) {
        ...
        this.busService.subscribe(channel, (message) => {
            console.log("Bus message received:", message);
            // Odoo bus messages come as [channel, message_type, payload]
            if (Array.isArray(message) && message.length >= 3) {
                const [msgChannel, msgType, payload] = message;
                if (msgType === "amp_task_update") {
                    this._onBusMessage(payload);
                }
            } else if (message && message.task_id) {
                // Direct payload format
                this._onBusMessage(message);
            }
        });
    ...
    _onBusMessage(payload) {
        console.log("Processing bus message payload:", payload);
        if (!payload) return;
